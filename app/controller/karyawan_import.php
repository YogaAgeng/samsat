...
//       membaca data (kolom ke-1 sd terakhir)
	  ...
	  $lokasi		  	= $data->val($i, 8);
	  $area			  	= $data->val($i, 9);
	  $sub_area		  	= $data->val($i, 10);
	  $start_date  		= $data->val($i, 11);
	  $end_date  		= $data->val($i, 12);

 
//      setelah data dibaca, masukkan ke tabel karyawan sql
      $query = "INSERT into karyawan (id, nik, nama, job_title, no_telp, jenis_kelamin, agama, lokasi, area, sub_area, start_date, end_date) values
	  ('NULL','$nik','$nama','$job_title','$no_telp', '$jenis_kelamin','$agama','$lokasi','$area','$sub_area','$start_date','$end_date')";
